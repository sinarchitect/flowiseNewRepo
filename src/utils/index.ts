...
export const sendRequest = async <ResponseData>(
  ...
  try {
    ...
    const reservedToken = localStorage.getItem('reservedT')

    const headers = {
      ...(reservedToken ? { Authorization: `Bearer ${reservedToken}` } : {}),
      ...((typeof params !== 'string' && isDefined(params.body)) ? { 'Content-Type': 'application/json' } : undefined),
    }
